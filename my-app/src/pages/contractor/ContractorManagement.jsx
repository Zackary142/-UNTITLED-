...
function QuoteCard({ card }) {
  ...
  const total = card.labourTotal + card.materialsTotal + card.additionalsTotal
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontFamily: font, fontSize: '13px', fontWeight: 600, color: 'var(--color-base-content)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {card.title || 'Untitled Quote'}
        </p>
        <p style={{ fontFamily: font, fontSize: '11px', color: 'var(--color-base-content)', opacity: 0.45, marginTop: '2px' }}>
          {card.FirstName || card.LastName ? `${card.FirstName} ${card.LastName}`.trim() + ' · ' : ''}
          {total > 0 ? `£${total.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'Draft'}
...
function CardModal({ onClose, onSave }) {
  ...
  function addMaterialRow() { setMaterials(p => [...p, { name: '', cost: '' }]) }
  function updateMaterial(i, key, val) { setMaterials(p => p.map((m, idx) => idx === i ? { ...m, [key]: val } : m)) }
  function removeMaterial(i) { setMaterials(p => p.filter((_, idx) => idx !== i)) }

  function addAdditionalRow() { setAdditionals(p => [...p, { description: '', amount: '' }]) }
  function updateAdditional(i, key, val) { setAdditionals(p => p.map((a, idx) => idx === i ? { ...a, [key]: val } : a)) }
  function removeAdditional(i) { setAdditionals(p => p.filter((_, idx) => idx !== i)) }

  function handleSave() {
    onSave({ title, FirstName, LastName, Email, Number, labourRate, labourHours, labourTotal, materials, materialsTotal, additionals, additionalsTotal, grandTotal })
  }
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: 'min(95vw, 820px)', maxHeight: '88vh',
        backgroundColor: 'var(--color-base-100)',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
          ...
          {/* Two-column layout: Customer | Project */}
          ...
          {grandTotal > 0 && (
            <div style={{
              ...
              backgroundColor: 'var(--color-base-200)',
              borderRadius: '10px', border: '1px solid var(--color-base-300)',
              ...
              <span style={{ fontFamily: font, fontSize: '18px', fontWeight: 700, color: 'var(--color-base-content)' }}>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--color-base-300)', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
          ...
          <button onClick={handleSave} style={{ padding: '9px 20px', borderRadius: '8px', border: 'none', backgroundColor: 'var(--color-base-content)', color: 'var(--color-base-100)', fontFamily: font, fontSize: '14px', fontWeight: 600, cursor: 'pointer' }}>Save quote</button>
        </div>
      </div>
    </>
  )
...
export default function ContractorManagement() {
  ...
  function handleSave(card) {
    setQuotes(prev => [...prev, { ...card, id: Date.now() }])
    setModalOpen(false)
  }
          ...
          <div className="bg-base-100 rounded-xl border border-base-300 p-4">
            ...
            {quotes.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '8px' }}>
                {quotes.map(card => <QuoteCard key={card.id} card={card} />)}
              </div>
            )}

            <button onClick={() => setModalOpen(true)} className="btn btn-ghost btn-sm w-full border border-dashed border-base-300 text-base-content/40 hover:text-base-content hover:border-base-400 mt-1">
        ...
        <div className="flex flex-col gap-3">
          <div className="bg-base-100 rounded-xl border border-base-300 p-4">
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-3 text-base-content/30">
              ...
              <p className="text-xs text-center leading-relaxed">No invoices yet.</p>
            </div>
